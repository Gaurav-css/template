...
"use client";
import React from 'react';
import { Stethoscope, Video, Search, MessageSquare, ShieldCheck, CalendarClock, AlertCircle, Heart, Repeat } from 'lucide-react';

const insights = [
  {
    icon: <ShieldCheck className="w-6 h-6 text-orange-500" />,
    title: "Preventive Care is Undervalued",
    description: "Most pet parents only go to the vet when something is wrong. Vets want to push preventive checkups, nutrition, exercise, and early warning signs.",
    angle: "Position your app as a “pet health partner” that reminds owners before it’s too late."
  },
  {
    icon: <CalendarClock className="w-6 h-6 text-orange-500" />,
    title: "Pet Parents Struggle with Compliance",
    description: "Owners often forget deworming, vaccinations, and flea/tick treatments. Missing these can lead to serious health issues.",
    angle: "Our app keeps track of these schedules and sends friendly nudges, so you'll never miss your pet’s important health dates."
  },
  {
    icon: <AlertCircle className="w-6 h-6 text-orange-500" />,
    title: "Owners Google Too Much, Too Late",
    description: "Vets often see frustrated pet parents who arrive with half-baked internet diagnoses, causing unnecessary stress.",
    angle: "Get vet-backed tips and quick access to professionals, a much safer alternative to random Google searches."
  },
  {
    icon: <Heart className="w-6 h-6 text-orange-500" />,
    title: "Emotional Connection Matters",
    description: "We know you treat your pets like family. Their health and happiness are a top priority.",
    angle: "Frame the app as “the trusted partner for your fur-baby’s wellbeing” using emotional and relatable storytelling."
  },
  {
    icon: <Repeat className="w-6 h-6 text-orange-500" />,
    title: "Aftercare & Follow-ups are Weak",
    description: "Vets are busy and often don’t have time to chase owners for crucial follow-up appointments or check-ins.",
    angle: "Our app bridges this gap, helping you track recovery, share updates with the vet, and get reminders for follow-up care."
  }
];

const InsightCard = ({ icon, title, description, angle }: { icon: React.ReactNode, title: string, description: string, angle: string }) => (
  <div className="bg-white dark:bg-zinc-800/50 p-6 rounded-2xl shadow-md border border-zinc-200 dark:border-zinc-700 flex flex-col h-full hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
    ...
    <p className="text-zinc-600 dark:text-zinc-400 text-sm mb-4 flex-grow">{description}</p>
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-900 text-zinc-800 dark:text-zinc-200">
      <main className="container mx-auto px-4 sm:px-6 lg:px-8 pt-24 md:pt-32 pb-16">
        ...
        <header className="text-center max-w-3xl mx-auto mb-12">
          ...
          <p className="mt-4 text-lg text-zinc-600 dark:text-zinc-400">
            Leveraging professional vet insights to help you provide the best care for your furry family members.
          </p>
        </header>

        {/* Vet Insights Section */}
        <section className="mb-16">
            <h2 className="text-3xl font-bold text-center mb-10 text-zinc-900 dark:text-white">Actionable Insights for Pet Parents</h2>
